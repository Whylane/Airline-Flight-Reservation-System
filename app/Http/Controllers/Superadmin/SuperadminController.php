...
class SuperadminController extends Controller
{
    public function index()
    {
        ...
        $userNotification = app(NotificationController::class)->getUserNotifications();

        return view('superadmin.index', compact('airlines', 'flights', 'airports', 'todayFlights', 'delayedFlights', 'totalPassengers', 'totalTicketAmount', 'users', 'userNotification'));
    }

    public function user()
    {
        // Retrieve all users with role 'user' or 'admin'
        $users = User::whereIn('role', ['user', 'admin'])->get();

        $userNotification = app(NotificationController::class)->getUserNotifications();

        return view('superadmin.user.index', compact('users', 'userNotification'));
    }
